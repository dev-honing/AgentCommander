...
## 11. REST 인증과 브라우저 — §9.3

**문제**: §9.3은 "로컬 단계부터 API Key 헤더 인증 적용"을 확정했다. 그런데 프론트엔드는 브라우저에서 도는 SPA다. 브라우저가 `X-API-Key`를 보내려면 키가 브라우저에 있어야 하고, Next.js에서 그러려면 `NEXT_PUBLIC_` 변수로 내려보내야 한다 — 그 순간 키가 번들에 박혀 **페이지를 여는 누구나 읽을 수 있다**. 인증이 형식만 남고 실질이 사라진다.

명세는 REST를 "설정성 작업용"으로만 그려서 이 충돌을 다루지 않는다. 하지만 Phase 5의 로그 뷰어처럼 화면이 직접 REST를 읽어야 하는 순간 바로 부딪힌다.

**채택**: Next.js 서버 라우트를 프록시로 둔다.

```
브라우저 → /api/backend/... (같은 오리진, 키 없음)
        → Next 서버가 BACKEND_API_KEY 를 붙여 전달
        → FastAPI /api/... (키 검증)
```

- 키는 `BACKEND_API_KEY`로 **서버에서만** 읽는다. `NEXT_PUBLIC_` 접두사를 붙이지 않는다.
- 브라우저 코드(`frontend/lib/api.ts`)는 키의 존재조차 모른다.
- AWS 전환 시 `BACKEND_URL`만 바꾸면 된다. §8.1의 "환경변수만 갈아끼운다" 원칙과 어긋나지 않는다.

부수 효과로, 백엔드를 외부에 직접 노출하지 않아도 되는 구조가 된다. Phase 7b에서 WebSocket에 인증을 붙일 때 REST 쪽은 이미 정리되어 있다.

### 누락된 헤더는 401로

`Header(...)`로 필수 선언하면 헤더 누락 시 FastAPI가 **422**를 돌려준다. 인증 실패는 "요청 형식 오류"가 아니라 "권한 없음"이므로, 헤더를 선택으로 받고 `auth.py`에서 401로 명확히 응답하도록 고쳤다.
...
명세 §8.1과 §9.3은 `AUTH_TOKEN` 환경변수 하나를 두고, 값이 비어 있으면 검증을 건너뛰는 방식을 예고했다. 실제로는 두 가지를 바꿨다.

**1. 토큰 직접 전달 대신 1회용 티켓을 쓴다.**

브라우저의 `WebSocket` 생성자는 핸드셰이크에 헤더를 붙일 수 없다. REST에서 쓰는 `X-API-Key`를 그대로 옮길 방법이 없다는 뜻이다. 남는 선택지는 쿼리스트링인데, 거기에 장기 토큰을 실으면 서버 접근 로그와 브라우저 히스토리에 영구히 남는다. REST 인증을 프록시 뒤로 숨겨 키가 브라우저에 닿지 않게 한 것(아래 "REST 인증과 브라우저" 항목)과 정면으로 어긋난다.

그래서 API Key로 잠긴 `POST /api/ws-ticket`에서 30초짜리 1회용 티켓을 받아 그것만 쿼리스트링에 싣는다. 키는 여전히 Next 프록시 서버에만 있고, 로그에 남는 값은 이미 소모되어 무효다. 재연결할 때마다 새로 받는다.

**2. 검증을 끄는 설정을 두지 않았다.**

`AUTH_TOKEN`이 비면 건너뛰는 설계는 "기본적으로 꺼져 있는 보호"다. 이 항목이 열려 있던 이유 자체가 그런 종류의 미룸이었고, 외부 공개 직전에 설정을 켜는 것을 잊으면 그대로 노출된다. `AUTH_TOKEN` 설정은 제거했고 인증은 항상 켜져 있다.

**함께 넣은 것 — Origin 검증.** CORS는 WebSocket에 적용되지 않는다. 미들웨어에 `ws_allowed_origin`을 넣어 뒀다고 해서 WebSocket이 보호되지는 않는다는 뜻이다. 핸드셰이크의 `Origin` 헤더를 직접 확인해, 악성 페이지가 방문자의 브라우저를 통해 붙는 것을 막는다. `Origin`이 아예 없는 요청(브라우저가 아닌 클라이언트)은 통과시킨다 — 위조가 가능해 검사에 의미가 없고, 어차피 티켓이 필요하다.
...
- ~~**WebSocket 인증 시점**~~ — **Phase 7a 이전에 앞당겨 적용 완료.** §9장 주의사항이 지적한 "설정은 막혔는데 상태는 누구나 보는" 비대칭을 닫았다. 다만 방식은 명세가 예고한 `AUTH_TOKEN` 방식이 아니다 — 아래 "WebSocket 인증 방식" 항목 참고.
